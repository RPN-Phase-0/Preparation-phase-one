...
function ganjilGenapAsianGames(date, data) {
  ...
  for(let i = 0; i < data.length; i++){
    let number = data[i].plat.split(" ");
    number = number[1].split("").pop();
    if(parseInt(number) % 2 === 0 && date % 2 === 0 && data[i].type == "Mobil" ){
      count++;
    } else if(parseInt(number) % 2 === 1 && date % 2 === 1 && data[i].type == "Mobil" ){
      count++;
    }
  }
  return count;
