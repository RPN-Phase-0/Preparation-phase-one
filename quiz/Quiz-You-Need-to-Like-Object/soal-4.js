...
function travelingIndonesia(arr, emoney) {
  ...
  for (let i = 0; i < arr.length; i++) {
    const travelDetail = arr[i].split("-");
    const name = travelDetail[0];
    const departureCity = travelDetail[1];
    const destinationCity = travelDetail[2];
    const transport = travelDetail[3];

    let baseCost = transportCosts[transport];

    const discount = baseCost * discounts[emoney];
