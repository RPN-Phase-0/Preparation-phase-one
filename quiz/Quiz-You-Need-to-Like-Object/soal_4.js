...
function travelingIndonesia(arr, emoney) {
  ...
  arr.forEach(item => {
    ...
    const departureCity = details[1];
    const destinationCity = details[2];
    const transport = details[3];
    
    let cost = transportCost[transport];
    let discountPercentage = discount[emoney];
    
    let totalCost = cost - (cost * discountPercentage);
  ...
  // Find the traveler with the highest total cost
