...
function squareNumber(num) {
  ...
  if(num < 3){
    ...
    for(let j = 0; j < second.length; j+= num){
      firstArray.push(second.slice(j, j+num))
    }
    for(let k = 0; k < firstArray.length; k++){
      if(k % 2 == 1){
        firstArray[k].sort((a, b) => b - a)
      }
    }
    for(let l = 0; l < firstArray.length; l++){
      for(let m = 0; m < firstArray[l].length; m++){
        if(firstArray[l][m] % 4 == 0){
          firstArray[l][m] = '#'
        }else if(firstArray[l][m] % 2 == 0 && firstArray[l][m] % 4 != 0){
          firstArray[l][m] = 'o'
        }else{
          firstArray[l][m] = 'x'
        }
