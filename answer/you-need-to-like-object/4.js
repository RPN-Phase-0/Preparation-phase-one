            ...
            for (let j = 0; j < rute.length; j++) {
                if (departureCity === rute[j]) {
                    for (k = 0; k < rute.length; k++) {
                        jarak++;
                        if (destinationCity === rute[k]) {
                            break;
                        }
                    }
                    if (j > jarak) {
                        totalCost = kendaraan[kendaraanUser] * (j - jarak);
                    } else {
                        totalCost = kendaraan[kendaraanUser] * (jarak - j);
                    }
                }
            }
            
        totalCost -= totalCost * diskon[emoney];
        result.push({
            name: name,
            departureCity: departureCity,
            destinationCity: destinationCity,
            transport: kendaraanUser,
            totalCost: totalCost
        })
    }
    return result;
