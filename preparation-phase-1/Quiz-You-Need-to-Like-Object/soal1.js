...
function ganjilGenapAsianGames(date, data) {
  ...
  for(let i = 0; i < data.length; i++) {
    let cariPlat = data[i].plat.split(" ")[1].slice(-1);
    let typeMobil = data[i].type;

    if(typeMobil === "Mobil" && (cariPlat % 2 === 0) && (date % 2 !== 0)) {
      count++
    } else if (typeMobil === "Mobil" && (cariPlat % 2 !== 0) && (date % 2 === 0)) {
      count++;
    } 
  }
